...
指标下拉新增「日内均价」，有成交量时使用典型价格加权，黄金无统一现货成交量时使用日内典型价格均值作为代理。左侧判断因子新增「日内均价」与「DXY对照」：价格与均价方向一致才给均价票，黄金与DXY反向运行才给宏观辅助票，同向时保持中性。K线解析保留成交量，为有成交量品种提供加权均价基础。详见[发布说明](docs/releases/release-notes.md)。

### 2026-08-28

指标下拉新增「超级趋势」与「箱体震荡」。超级趋势是 ATR 包络趋势跟踪线，多头段画在价格下方、空头段画在价格上方，收盘穿越才换向并标出转多 / 转空，周期（7 / 10 / 14）与倍数（2 / 2.5 / 3）在专属设置栏里调。箱体震荡把摆动高低点按 ATR 容差聚类成横向箱体，画出上下沿、中轴与触碰点，状态条给出高度、触碰次数和现价在箱体里的位置，回看根数可选 60 / 120 / 180。两者的翻转与破位都会进时间信号线。

主图底部增加「时间信号线」（顶栏「信号线」开关）：把企稳、开单、结构与破位等事件画成底部色点，悬停看说明，点击跳到对应 K 线。顶栏「布局」提供标准盯盘 / 裸K专注 / 布林+SMC / 开单跟单四套预设，并可「保存当前」。详见[发布说明](docs/releases/release-notes.md)。
